...
OUTPUT_DIR = os.path.abspath(os.path.join(BASE_DIR, '../../data-engine/provas'))
...
def parse_pagina_prova(url):
    ...
    # PDFs (visualizar - order matters: 1st=prova, 2nd=gabarito)
    ...
    if len(pdf_items) >= 1:
        prova_pdf = pdf_items[0].get("data-pdf-share") or pdf_items[0].get("href")
    if len(pdf_items) >= 2:
        gabarito_pdf = pdf_items[1].get("data-pdf-share") or pdf_items[1].get("href")
...
def processar_prova(url, index):
    ...
    # 🔥 se já existe E tem JSON, pula (prova completa)

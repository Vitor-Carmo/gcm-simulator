...
const MODOS = [
  {
    ...
    icon: BookOpen,
    label: "Por Prova",
    desc: "Simule uma prova completa",
  },
  {
    id: "materia",
    icon: Filter,
    label: "Por Matéria",
    desc: "Foque em uma disciplina",
  },
  {
    id: "banca",
    icon: Shield,
    label: "Por Banca",
    desc: "Questões de uma banca específica",
  },
  {
    id: "misto",
    icon: Shuffle,
    label: "Aleatório",
    desc: "Mix de todas as provas",
  },
];
    ...
    <div className="max-w-[430px] mx-auto min-h-screen bg-[#0f172a] flex flex-col">
      ...
      <div className="px-5 pt-12 pb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[11px] text-blue-400 uppercase tracking-widest font-medium">
            GCM Simulador
          </span>
        </div>
        ...
        <p className="text-[13px] text-slate-500 mt-1">
          {filtros.provas.length} provas · {filtros.materias.length} matérias
        </p>
      </div>
          ...
          {MODOS.map((m) => {
            ...
            return (
              ...
              >
                <Icon
                  size={18}
                  className={ativo ? "text-blue-400" : "text-slate-500"}
                />
                <div>
                  <p
                    className={`text-[13px] font-semibold ${ativo ? "text-blue-300" : "text-slate-200"}`}
                  >
                    {m.label}
                  </p>
            ...
            <div className="flex flex-col gap-2">
              {historico.slice(0, 5).map((h) => {
                ...
                return (
                  <div
                    key={h.id}
                    className="flex items-center gap-3 bg-[#1e293b] border border-slate-700/30 rounded-xl px-4 py-3"
                  >
                    <BarChart2
                      size={15}
                    ...
                    <span
                      className={`text-[13px] font-semibold ${pct >= 70 ? "text-green-400" : pct >= 50 ? "text-amber-400" : "text-red-400"}`}
                    >
                      {pct}%
                    </span>
                  </div>
      ...
      <footer className="w-full py-6 mt-auto text-center border-t border-slate-800">
        <p className="text-slate-500 text-xs tracking-widest uppercase">
          Desenvolvido com ❤️ por Vitor
        </p>
        <p className="text-slate-400 text-sm mt-1 italic">
          Projeto feito para o amor da minha vida Larissa Pires.
        </p>
      </footer>
    </div>
  );
